...
def file_process():
    ...
    for one in file:
        id = one['id']
        file_id.append(id)
        file_list.append(one[qaField])
        id2article[id] = one

    # 读取文档的json文件
    # with open("./text.json",'r',encoding='utf-8') as fp:
    #     json_data = json.load(fp)
    # file_id = list(json_data.keys())
    # file_content = list(json_data.values())
    # corpus = file_content
    # query = '资金检查的工作方案'
    # 检索文章的文件夹
    # path = './测试文本'
    ...
    # 进行相似度计算
    # res = m.most_similar(queries, topn=3)
    # res = m.most_similar(query)
    # {0: {4: 1.0, 2: 0.4285055994987488, 3: 0.34824615716934204, 0: 0.3471887409687042, 1: 0.07818949222564697}}
    # res_1 = res[0]
    #摘要的相似度
    # similar_value = list(res_1.values())
    # file_id_new = list(res_1.keys())
    # similar_values = list(res_1.values())
    # print('sim search: ', res)
    # for q_id, c in res.items():
    #     print('query:', query[q_id])
    #     print("search top 3:")
    #     for corpus_id, s in c.items():
    #         print(f'\t{m.corpus[corpus_id]}: {s:.4f}')
    # print('-' * 50 + '\n')

    # res = list(zip(file_id, abstract_content, similar_value))
    # logger.info(query)
    # logger.info(res)
    # for i in range(len(file)):
    #     for j in range(len(abstract_content)):
    #         one = file[i]
    #         one['zzy_abstract'] = abstract_content[j]
    #         i += 1
    #         j += 1
    #         break
